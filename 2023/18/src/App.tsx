    ...
    <div className="App">
      ...
      <div className={`${ displayForm ? 'display-form' : 'display-none'  }`}>
        <form onSubmit={(e)=>addGiftHandler(e)}>
          ...
          <label className="" htmlFor="precio">Precio</label>
          <input type="number" value={priceInput} placeholder="Precio" name="precio" id="" onChange={ (e) => setPriceInput(parseInt(e.target.value)) }/>
          <label htmlFor="cantidad" className="">Cantidad</label>
          <input type="number" name="cantidad" value={amountInput} id="" placeholder="1" onChange={ (e) => setAmountInput(parseInt(e.target.value)) } required />
            ...
            <div className="data-regalo-container">
              ...
              <img className="image-thumbnail" src={regalo.imagen ? regalo.imagen : 'https://res.cloudinary.com/dc6mrv5cb/image/upload/v1702233464/adviency-goncy/default_image.512x512_hd7edh.png'} alt={`Imagen ${regalo.nombre}`}/>
                <div className="nombre-cantidad-destinatario-container">
                  <span>{regalo.nombre}  ({regalo.cantidad}) - ${regalo.cantidad * regalo.precio}</span>
                  {
      ...
      { !isLoading && (<><hr style={{width:'100%'}} /><h2 className="precio-total">Total: <span>${precioTotal}</span></h2></>)}
